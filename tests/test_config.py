...
from jev_hermes import config, tools  # noqa: E402
...
class ConfigTests(unittest.TestCase):
    ...
    def test_env_override_and_coercion(self):
        with mock.patch.dict(os.environ, {"JEV_FILTER_OUTPUT": "1", "JEV_FILTER_MIN_CHARS": "9000"}):
    ...
    def test_env_beats_settings(self):
        ...
        with mock.patch.dict(os.environ, {"JEV_ROUTING_HINT": "true"}):
            self.assertEqual(config.get("routing_hint"), (True, "env"))

    def test_api_settings_defaults(self):
        env = {k: v for k, v in os.environ.items() if not k.startswith("JEV_")}
        with mock.patch.dict(os.environ, env, clear=True):
            self.assertEqual(config.get("base_url"), ("https://api.typesafe.ai/v1", "default"))
            self.assertEqual(config.get("endpoint_path"), ("/systemone", "default"))
            self.assertEqual(config.get("api_key_env"), ("TYPESAFE_API_KEY", "default"))
            self.assertEqual(config.get("jev_model"), ("jev-latest", "default"))
            self.assertEqual(config.get("request_timeout_s"), (30, "default"))
            self.assertEqual(config.get("result_excerpt_chars"), (500, "default"))
...
class SlashConfigTests(unittest.TestCase):
    ...
    def test_config_lists_settings(self):
        ...
        self.assertIn("api_key_env", keys)

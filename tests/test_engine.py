"""Unit tests for the Jev context engine — stand-in base class, stubbed predict."""
...
from jev_hermes import engine as engine_mod  # noqa: E402
from jev_hermes.engine import JevContextCompressor  # noqa: E402
...
def predict_with(keep_call, keep_result):
    return ({"answers": {"keep_call": {"noul": keep_call},
                         "keep_result": {"noul": keep_result}}}, 5.0, "jev", "jev-latest")


class EngineTests(unittest.TestCase):
    def setUp(self):
        self.engine = JevContextCompressor()
    ...
    def test_all_kept_falls_back_on_no_progress(self):
        with mock.patch.object(engine_mod.client, "predict",
        ...
        self.assertEqual(self.engine.jev_fallbacks, 1)
    ...
    def test_drop_result_truncates_with_marker(self):
        ...
        self.assertTrue(all("[jev-compaction truncated" in m["content"] for m in tool_rows))
    ...
    def test_error_results_get_lower_keep_bar(self):
        ...
        with mock.patch.object(engine_mod.client, "predict", side_effect=judge):
    ...
    def test_reduction_gate_falls_back(self):
        ...
        with mock.patch.object(engine_mod.client, "predict", side_effect=lambda *a, **k: next(calls)):
            out, n = self._prune(make_transcript(pairs=12, result_len=3000))
        self.assertEqual(self.engine.jev_fallbacks, 1)

    def test_predict_exception_falls_back(self):
        with mock.patch.object(engine_mod.client, "predict", side_effect=RuntimeError("down")):
            out, n = self._prune(make_transcript())
        self.assertEqual(self.engine.jev_fallbacks, 1)
        self.assertEqual(len(out), len(make_transcript()))  # stand-in fallback = unchanged

    def test_proactive_path_bypasses_jev(self):
        with mock.patch.object(engine_mod.client, "predict") as p:
    ...
    def test_bad_probability_falls_back(self):
        bad = ({"answers": {"keep_call": {"noul": 1.7}, "keep_result": {"noul": 0.5}}},
               1.0, "jev", "jev-latest")
        with mock.patch.object(engine_mod.client, "predict", return_value=bad):
            out, n = self._prune(make_transcript())
        self.assertEqual(self.engine.jev_fallbacks, 1)
    ...
    def test_state_fits_small_budget(self):
        ...
        import json
        self.assertLess(len(json.dumps(state)), 4000)  # comfortably inside Jev's window

    def test_deepcopy(self):
        import copy
        self.engine.jev_calls = 7
        dup = copy.deepcopy(self.engine)
        self.assertEqual(dup.jev_calls, 7)
        self.assertEqual(dup.name, "jev")

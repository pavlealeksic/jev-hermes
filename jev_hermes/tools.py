"""Tool handlers for the Jev Hermes plugin.
...
def _run_decide(state: Any, questions: Dict[str, Any], model: Optional[str],
    ...
    result, latency_ms, used_backend, used_model = client.predict(
...
def handle_decide(args: Dict[str, Any], **kwargs: Any) -> str:
    ...
    try:
        ...
        if preset:
            questions = client.get_preset(str(preset), backend=backend_name)
        else:
    ...
    except client.BackendUnavailableError as exc:
        return _err(str(exc), hint="Run jev_status for API diagnostics.")
...
def handle_status(args: Dict[str, Any], **kwargs: Any) -> str:
    """jev_status tool handler."""
    try:
        diag = client.diagnostics()
        selected_engine = _selected_context_engine()
        return _dump({
            "success": True,
            "ready": diag["key_configured"],
            "api": diag,
            "context_engine": {
                "registered_as": "jev",
                "selected": selected_engine,
                "active": selected_engine == "jev",
                "enable": "hermes config set context.engine jev   (then /reset)",
            },
            "settings": config.describe(),
            "metrics": metrics.snapshot(),
            "setup": (
                None if diag["key_configured"]
                else client._setup_hint(diag["api_key_env"])
            ),
        })
...
def handle_keycheck() -> str:
    """Report whether the configured API key env var is set (never prints it)."""
    diag = client.diagnostics()
    if diag["key_configured"]:
        return _dump({
            "success": True,
            "key_configured": True,
            "message": f"{diag['api_key_env']} is set; requests will go to "
                       f"{diag['base_url']}{diag['endpoint_path']} as {diag['model']}.",
        })
    return _dump({
        "success": False,
        "key_configured": False,
        "error": f"{diag['api_key_env']} is not set.",
        "setup": client._setup_hint(diag["api_key_env"]),
    })
...
def handle_config() -> str:
    ...
    return _dump({"success": True,
                  "settings": config.describe(),
                  "usage": "/jev set <key> <value> — e.g. /jev set filter_output true"})
...
def handle_set(raw: str) -> str:
    ...
    if len(tokens) != 2 or not tokens[0] or not tokens[1].strip():
        return _err(f"Usage: /jev set <key> <value>. Valid keys: {', '.join(sorted(config.SETTINGS))}")
...
def handle_slash(raw_args: str) -> str:
    ...
    Forms:
      /jev status | stats | keycheck | config
      /jev set <key> <value>
      /jev preset <router|guard|moderation|triage>; <state>
      /jev noul; <yes/no question>; <state>
      /jev choice|score; <instructions>; <opt1, opt2, ...>; <state>
    """
    ...
    if not raw or raw == "help":
        return _dump({
            ...
            "usage": [
                ...
                "/jev keycheck",
                "/jev config",
                "/jev set filter_output true",
                "/jev preset triage; <text>",
                "/jev noul; Does the user ask for a refund?; <text>",
                "/jev choice; Which department?; billing, technical, other; <text>",
                "/jev score; How urgent?; low, medium, high, critical; <text>",
            ],
        })
    if raw == "status":
    ...
    if head == "preset" and len(parts) >= 2:
        ...
        if len(tokens) != 2:
            return _err("Usage: /jev preset <router|guard|moderation|triage>; <state>")
    ...
    if qtype not in _QUESTION_TYPES:
        return _err(f"Unknown form. First segment must be 'status', 'preset', or one of "
                    f"{', '.join(_QUESTION_TYPES)}. Try /jev help.")

    if qtype == "noul":
        if len(parts) < 3:
            return _err("Usage: /jev noul; <yes/no question>; <state>")
        questions = {"answer": {"type": "noul", "instructions": parts[1]}}
        state = "; ".join(parts[2:])
    else:
        if len(parts) < 4:
            return _err(f"Usage: /jev {qtype}; <instructions>; <opt1, opt2, ...>; <state>")

"""Jev-powered context engine for Hermes Agent.

Design follows ``hermes-jev-compact`` (TheEpTic, MIT) — same seam (override
``_prune_old_tool_results`` on the built-in ContextCompressor), same decision
ladder (keep / drop_result / drop_call), same commit gates (validity + minimum
reduction + deterministic fallback). Judgments come from the hosted Jev
typed-decision API: Jev's context window is huge (~25k tokens vs Laya's ~1k),
so each verdict can see a larger excerpt — but every candidate still costs one
paid API call (~70–500 ms) and the judged excerpt leaves the machine.
...
try:
    from . import client, config, metrics
...
_TRUNCATION_MARKER = (
    "[jev-compaction truncated {removed} chars of this tool result{error_note}; "
...
def _redact(text: str) -> str:
    ...
    Matters even more here than in the local Laya plugin: whatever passes this
    filter is sent to the hosted API.
    """
    try:
...
class JevContextCompressor(_Base):
    """ContextCompressor whose phase-1 prune is targeted by Jev API decisions."""

    @property
    def name(self) -> str:
        return "jev"

    def __init__(self, model: str = "jev", **kwargs: Any) -> None:
        ...
        self.jev_calls = 0             # Jev API calls made for compaction
        self.jev_pruned_units = 0      # units truncated or dropped
        self.jev_kept_units = 0
        self.jev_fallbacks = 0         # times the built-in prune ran instead
    ...
    def _prune_old_tool_results(self, messages, protect_tail_count,
        ...
        applied = None
        try:
            applied = self._jev_prune(messages, protect_tail_count,
                                      protect_tail_tokens, min_prune_chars)
        except Exception as exc:
            logger.info("jev compaction aborted (%s); built-in prune", exc)
        if applied is None:
    ...
    def _judge(self, messages: List[Dict[str, Any]], c: _Candidate) -> Tuple[float, float]:
        """One Jev call per candidate, both questions in a single request."""
        questions = {
            "keep_call": {"type": "noul", "instructions": _QUESTION_KEEP_CALL},
            "keep_result": {"type": "noul", "instructions": _QUESTION_KEEP_RESULT},
        }
        result, _, _, _ = client.predict(self._build_state(messages, c), questions)
        self.jev_calls += 1
        ...
        for value in (keep_call, keep_result):
            if isinstance(value, bool) or not isinstance(value, (int, float)):
                raise ValueError(f"jev returned no usable probability: {value!r}")
            if not 0.0 <= float(value) <= 1.0:
                raise ValueError(f"jev probability out of range: {value!r}")
        return float(keep_call), float(keep_result)

    def _jev_prune(self, messages, protect_tail_count, protect_tail_tokens,
        ...
        if not _valid_openai_sequence(applied):
            logger.info("jev compaction invalid transcript; built-in prune")
            return None
        ...
        if ratio < float(config.get_value("min_reduction_ratio")):
            logger.info("jev compaction reduction %.2f under gate; built-in prune", ratio)
            return None
        self.jev_pruned_units += units
        self.jev_kept_units += sum(1 for d in decisions.values() if d == "keep")
        metrics.record_compaction(units, before - after)
        if not getattr(self, "quiet_mode", False):
            logger.info("jev compaction: %d units pruned, %d kept, %.0f%% smaller",
                        units, self.jev_kept_units, ratio * 100)

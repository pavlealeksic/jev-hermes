"""Opt-in output filtering hooks for the Jev Hermes plugin (default OFF).

Enabled with the ``filter_output`` setting (``/jev set filter_output true`` or
``JEV_FILTER_OUTPUT=1``). When a *successful* command/tool produces very large
output (``filter_min_chars``, default 6000), Jev decides whether the output looks
disposable (progress bars, install spam, boilerplate). Only when Jev is confident
...
Jev is non-generative — this truncates, it never summarizes. Both hooks are
fail-open: any exception returns None so Hermes keeps the original output.

Note: unlike the local Laya plugin, each judgment is a paid hosted API call and
the probed excerpt leaves the machine — another reason this defaults to OFF.
"""
...
def _maybe_truncate(output: Any, context: str) -> Optional[str]:
    ...
    return (
        output[:_HEAD_CHARS]
        + f"\n\n[jev: truncated {removed} chars of low-value output; "
...
def transform_terminal_output(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        logger.debug("jev terminal filter skipped: %s", exc)
        return None
...
def transform_tool_result(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        metrics.record_error()
        logger.debug("jev tool-result filter skipped: %s", exc)
        return None

"""HTTP client for the hosted Jev typed-decision API ("System One").

Pure stdlib ``urllib`` — zero dependencies, nothing to install. The wire shape:
``POST {base_url}{endpoint_path}`` with JSON ``{"model", "state", "questions"}``
returns ``{"answers": {<name>: {"noul": 0..1} | {"choice", "probabilities"} |
{"score", ...}}}``. Reference deployment: TypeSafe (``https://api.typesafe.ai/v1``,
``/systemone``, key in ``TYPESAFE_API_KEY``, model ``jev-latest``). OpenRouter
speaks the same shape (``https://openrouter.ai``, ``/api/alpha/decisions``,
``OPENROUTER_API_KEY``, model ``typesafe/jev-1.13``).

URL policy ported from ``hermes-jev-compact`` (asker.py): https anywhere,
cleartext http only for loopback/private LAN addresses, no userinfo, no
redirects (a redirect would carry the bearer key somewhere unvetted), response
capped at 1 MB, endpoint path restricted to ``[A-Za-z0-9/._-~]+``. All failures
raise :class:`BackendUnavailableError`; callers fall back or fail open.
"""
...
def _setup_hint(env_name: str) -> str:
    return (
        f"Set the {env_name} environment variable to your API key and restart "
        f"Hermes (e.g. export {env_name}=...). TypeSafe: base_url "
        f"{DEFAULT_BASE_URL}, endpoint_path {DEFAULT_ENDPOINT_PATH}, model "
        f"{DEFAULT_MODEL}. OpenRouter speaks the same shape: base_url "
        f"https://openrouter.ai, endpoint_path /api/alpha/decisions, "
        f"api_key_env OPENROUTER_API_KEY, jev_model typesafe/jev-1.13."
    )


# -- URL policy (ported from hermes-jev-compact asker.py) ----------------------
...
# Module-level so tests and smoke scripts can stub the network with mock.patch.
...
def _api_key() -> str:
    env_name = str(_config.get_value("api_key_env")).strip() or DEFAULT_API_KEY_ENV
    key = os.environ.get(env_name, "").strip()
    if not key:
        raise BackendUnavailableError(
            f"Jev API key not configured ({env_name} is not set). {_setup_hint(env_name)}"
        )
    return key
...
def _timeout() -> float:
    try:
        value = float(_config.get_value("request_timeout_s"))
    except (TypeError, ValueError):
        raise BackendUnavailableError("invalid jev request_timeout_s") from None
    if not math.isfinite(value) or value <= 0:
        raise BackendUnavailableError("invalid jev request_timeout_s")
    return value
...
def predict(
    ...
    """One typed-decision call. Returns (raw_result, latency_ms, "jev", model).

    No retries — a single POST, fail closed on any transport/validation error.
    """
    if backend not in (None, "", "auto", BACKEND_NAME):
        raise ValueError(f"Unknown backend {backend!r}; this plugin only talks to 'jev'.")
    used_model = (model or str(_config.get_value("jev_model"))).strip() or DEFAULT_MODEL
    url = _join_url(
        str(_config.get_value("base_url")), str(_config.get_value("endpoint_path"))
    )
    _check_url(url)  # fail closed before the key is attached to anything
    body = _build_body(used_model, state, questions)
    headers = {
    ...
    try:
        status, text = _transport(url, body, headers, _timeout())
    ...
    return result, latency_ms, BACKEND_NAME, used_model


def get_preset(name: str, backend: Optional[str] = None) -> Dict[str, Any]:
    """Return one of the bundled question presets (router/guard/moderation/triage).

    Unlike Laya, the hosted API has no server-side presets — these are inline
    data shipped with the plugin (ported verbatim from laya-mlx presets.py).
    """
    try:
        from . import presets
    except ImportError:  # standalone import (tests, smoke scripts)
...
def diagnostics() -> Dict[str, Any]:
    """Config + key presence for status reporting (never leaks the key)."""
    env_name = str(_config.get_value("api_key_env")).strip() or DEFAULT_API_KEY_ENV
    return {
        "backend": BACKEND_NAME,
        "base_url": str(_config.get_value("base_url")),
        "endpoint_path": _normalize_endpoint_path(str(_config.get_value("endpoint_path"))),
        "model": str(_config.get_value("jev_model")),
        "request_timeout_s": float(_config.get_value("request_timeout_s")),
        "api_key_env": env_name,
        "key_configured": bool(os.environ.get(env_name, "").strip()),
    }
